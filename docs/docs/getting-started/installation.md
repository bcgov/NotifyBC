...
The above commands installs the _main_ version, i.e. main branch tip of _NotifyBC_ GitHub repository. To install a specific version, say _v6.3.0_, run

```sh
 git checkout tags/v6.3.0 -b v6.3.0
```
...
### _NotifyBC_ chart as dependency

If your application has a Helm chart, you can set _NotifyBC_ chart as its dependency.
...
In such case all customizations above have to be enclosed in _notify-bc_.

To add _NotifyBC_ repo, run

```sh
helm repo add notify-bc https://bcgov.github.io/NotifyBC/helm
```
